...
=begin
I think another example method would be helpful here. We'll call this one englishNumber. 
It will take a number, like 22, and return the english version of it (in this case, the string 'twenty-two'). 
For now, let's have it only work on integers from 0 to 100.
(NOTE: This method uses a new trick to return from a method early using the return keyword, and introduces a new twist on branching:
...
def english_number(number)
  ...
  if write > 0
    if write == 1   # Uh-oh...
      # Since we can't write "tenty-two" instead of "twelve", we have to make special accomodations for these:
      if    left == 0
